...
import folium
from folium import PolyLine, Marker, Icon, Popup, Element

# ---------------------------------------------------------------------------
# 1. Load OSM track geometry
# ---------------------------------------------------------------------------
osm_data = json.loads(
    Path(r"C:\Users\yulel\AppData\Local\Temp\opencode\trenmaya_osm.json").read_text(encoding="utf-8")
)
track_ways = [e for e in osm_data["tracks"]["elements"] if e.get("type") == "way" and e.get("geometry")]
...
def snap_to_track(lat: float, lon: float) -> tuple:
    best_d = float("inf")
    best_pt = (lat, lon)
    for pt in all_track_points:
...
def estimate(lat, lon):
    """Snap estimated coords to the nearest track point so markers sit on the route."""
    return snap_to_track(lat, lon)

# ---------------------------------------------------------------------------
# 2. Full station list (counter-clockwise from Cancún)
#    Source notes:
#      "wiki"   = coordinates from Wikipedia / Wikimedia
#      "osm-sp" = OSM stop_position node (railway=stop on the tracks)
#      "latln"  = latlong.info / other geo DB
#      "est"    = physically estimated, then snapped to track
# ---------------------------------------------------------------------------
# (name, lat, lon, day, overnight, key_site, wikipedia_url)
stations = [
    # Sections 4+5  —  Cancún and Caribbean coast
    ("Cancún Aeropuerto",     21.02808,  -86.87281,  1,   True,  False, "https://en.wikipedia.org/wiki/Canc%C3%BAn_Airport_railway_station"),
    ...
    ("Nicolás Bravo (Kohunlich)",  18.450,  -88.720,  None,False, False, ""),
    ...
    ("Escárcega",             18.60667,  -90.73444,  None,False, False, "https://en.wikipedia.org/wiki/Esc%C3%A1rcega"),
    ...
    ("Carrillo Puerto (Champotón)", 19.09283, -90.52310, None,False, False, ""),
    ("Edzná",                19.400,   -90.420,    None,False, False, "https://en.wikipedia.org/wiki/Edzn%C3%A1"),
    ...
    ("Hecelchakán",          20.16667,  -90.13333,  None,False, False, "https://en.wikipedia.org/wiki/Hecelchak%C3%A1n"),
    ("Calkiní",              20.367,    -90.050,    None,False, False, "https://en.wikipedia.org/wiki/Calkin%C3%AD"),

    # Section 3  —  eastern Yucatán to Izamal
    ("Maxcanú",              20.583,    -89.983,    None,False, False, "https://en.wikipedia.org/wiki/Maxcan%C3%BA"),
    ("Umán",                 20.84891,  -89.75603,  None,False, False, "https://en.wikipedia.org/wiki/Um%C3%A1n_railway_station"),
    ("Teya Mérida",          20.92883,  -89.51449,  9,   True,  False, "https://en.wikipedia.org/wiki/M%C3%A9rida,_Yucat%C3%A1n"),
    ("Tixkokob",             20.98704,  -89.39865,  None,False, False, ""),
    ("Izamal",               20.93608,  -89.08559,  None,False, False, "https://en.wikipedia.org/wiki/Izamal"),

    # Section 4  —  back east to Cancún
    ("Chichén-Itzá",         20.72160,  -88.54874,  10,  False, True,  "https://en.wikipedia.org/wiki/Chichen_Itza"),
    ("Valladolid",           20.74033,  -88.19312,  None,False, False, "https://en.wikipedia.org/wiki/Valladolid,_Yucat%C3%A1n"),
    ("Nuevo Xcán",           20.87907,  -87.60277,  None,False, False, "https://en.wikipedia.org/wiki/Nuevo_Xc%C3%A1n_railway_station"),
    ("Leona Vicario",        20.97966,  -87.19723,  None,False, False, ""),
]

# Snap estimated-only coords  (markers for Edzná, Conhuas, Nicolás Bravo, etc. that are rough estimates)
ROUGH_ESTIMATES = {"Edzná", "Conhuas", "Nicolás Bravo (Kohunlich)", "Limones"}
...
# ---------------------------------------------------------------------------
# 3. Key archaeological sites (separate markers for the actual ruins)
# ---------------------------------------------------------------------------
sites = [
    ...
    ("Chichén Itzá (ruins)  UNESCO",  20.6829,  -88.5686,  "https://en.wikipedia.org/wiki/Chichen_Itza"),
]
...
Marker(
    ...
    popup=Popup("<b>Arrive &amp; Depart</b><br>London ↔ Cancún (CUN)<br>Round-trip ~£450–600", max_width=300),
    tooltip="Cancún Airport  (CUN)",
...
folium.PolyLine(
    ...
    tooltip="Round-trip: London ↔ Cancún  (~10.5 h)",
).add_to(m)
...
<div style="position:fixed;top:10px;left:50%;transform:translateX(-50%);z-index:9999;
  ...
  <p style="margin:0;font-size:12px;color:#666;">
    ...
    <span style="color:red;">⬤</span> Overnight &nbsp;
    <span style="color:darkgreen;">⬤</span> Key site &nbsp;
    <span style="color:blue;">⬤</span> Station/Paradero &nbsp;
...
# Save
out = Path(r"C:\Users\yulel\tren-maya-trip\route-map.html")
m.save(str(out))
